...
=head1  Dernieres MAJ
  ...
  03/08/2011 - Creation
  26/10/2015 - Ajout ABS pour prise en compte termes negatifs possibles si approximation
...
for (0 .. $frequence_max-$frequence_min) {
	$pos = $pos + 19 + length($_);
	if ($pos >= 50) {print FIC "\n"," "x8;$pos=8;}
	print FIC "LOG10(ABS(X",$_,"/1000.))";
	if ($_ != $frequence_max-$frequence_min) {print FIC "+";}
}
print FIC "\n"," "x8,")/",($frequence_max-$frequence_min+1),".)";
if (contribution_ref($frequence_min,$frequence_max)<0) {
	printf FIC "+%0.5f;\n",abs(contribution_ref($frequence_min,$frequence_max));
}
else {
	printf FIC "-%0.5f;\n",contribution_ref($frequence_min,$frequence_max);
}
print FIC " "x8,"ISORAID = 10.**(6.-(INERT/20.))\n";
